...
pub(crate) const DEFAULT_SLAVE: u8 = 0x01;

/// V-SET — output voltage setpoint. Scale 100 (V).
pub(crate) const REG_V_SET: u16 = 0x0000;
/// I-SET — output current limit setpoint. Scale 100 (A).
pub(crate) const REG_I_SET: u16 = 0x0001;
/// V-OUT — measured output voltage. Scale 100 (V).
pub(crate) const REG_V_OUT: u16 = 0x0002;
/// I-OUT — measured output current. Scale 100 (A).
pub(crate) const REG_I_OUT: u16 = 0x0003;
/// P-OUT — measured output power. Scale 10 (W).
pub(crate) const REG_P_OUT: u16 = 0x0004;
/// V-IN — measured input voltage. Scale 100 (V).
pub(crate) const REG_V_IN: u16 = 0x0005;
/// AH-LOW — cumulative output charge, low word. Scale 1000 (Ah).
pub(crate) const REG_AH_LOW: u16 = 0x0006;
/// AH-HIGH — cumulative output charge, high word. Untested in
/// community docs; treat the 32-bit composition as best-effort.
pub(crate) const REG_AH_HIGH: u16 = 0x0007;
/// WH-LOW — cumulative output energy, low word. Scale 1000 (Wh).
pub(crate) const REG_WH_LOW: u16 = 0x0008;
/// WH-HIGH — cumulative output energy, high word.
pub(crate) const REG_WH_HIGH: u16 = 0x0009;
/// OUT-H — output-on time, hours.
pub(crate) const REG_OUT_H: u16 = 0x000A;
/// OUT-M — output-on time, minutes.
pub(crate) const REG_OUT_M: u16 = 0x000B;
/// OUT-S — output-on time, seconds.
pub(crate) const REG_OUT_S: u16 = 0x000C;
/// T-IN — internal temperature. Scale 10 (°C/°F per [`REG_TEMP_UNIT`]).
pub(crate) const REG_T_IN: u16 = 0x000D;
/// T-EX — external probe temperature. Connected-probe scale is unverified;
/// intentionally available only through raw register access.
pub(crate) const REG_T_EX: u16 = 0x000E;
/// LOCK — front-panel key lock (0 unlocked, 1 locked).
pub(crate) const REG_LOCK: u16 = 0x000F;
/// PROTECT — latched protection cause. Write 0 to clear.
pub(crate) const REG_PROTECT: u16 = 0x0010;
/// CVCC — regulation mode (0 CV, 1 CC).
pub(crate) const REG_CVCC: u16 = 0x0011;
...
pub(crate) const REG_S_LVP: u16 = GROUP_BASE + 2;
pub(crate) const REG_S_OVP: u16 = GROUP_BASE + 3;
pub(crate) const REG_S_OCP: u16 = GROUP_BASE + 4;
pub(crate) const REG_S_INI: u16 = GROUP_BASE + 13;
...
// `read_temperature_internal` (T_IN..TEMP_UNIT).
